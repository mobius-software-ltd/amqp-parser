...
public class AMQPClose extends AMQPHeader
{
	...
	public AMQPClose()
	{
		super(HeaderCode.CLOSE, 2, 0, 0);
	}

	private AMQPClose(HeaderCode code, int doff, int type, int channel, AMQPError error)
	{
	...
	public static class Builder extends AMQPHeaderBuilder<Builder>
	{
		...
		public AMQPClose build()
		{
			return new AMQPClose(HeaderCode.CLOSE, doff, type, channel, error);
		}
	...
	{
		return "AMQPClose [error=" + error + ", code=" + code + ", doff=" + doff + ", type=" + type + ", channel=" + channel + "]";
	}
